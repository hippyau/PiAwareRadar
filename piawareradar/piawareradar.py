...
parser.add_argument(
    "--screen", help="The screen config to use [normal / touch]")
parser.add_argument("--fullscreen", help="Fullscreen radar",
                    action="store_true")
...
# draw close button
close_rect = pygame.draw.rect(screen, BUTTONCOLOR, CLOSERECT, 2)
pygame.draw.line(screen, BUTTONCOLOR,
                 (CLOSERECT[0], CLOSERECT[1]), (CLOSERECT[0] +
                                                CLOSERECT[2], CLOSERECT[1] + CLOSERECT[3]),
                 1)
pygame.draw.line(screen, BUTTONCOLOR,
                 (CLOSERECT[0], CLOSERECT[1] + CLOSERECT[3]
                  ), (CLOSERECT[0] + CLOSERECT[2], CLOSERECT[1]),
                 1)
# draw scale plus button
scale_plus_rect = pygame.draw.rect(screen, BUTTONCOLOR, SCALEPLUSRECT, 2)
pygame.draw.line(screen, BUTTONCOLOR,
                 (SCALEPLUSRECT[0] + (SCALEPLUSRECT[2] / 2),
                  SCALEPLUSRECT[1] + 5),
                 (SCALEPLUSRECT[0] + (SCALEPLUSRECT[2] / 2),
                  SCALEPLUSRECT[1] + SCALEPLUSRECT[3] - 5),
                 1)
pygame.draw.line(screen, BUTTONCOLOR,
                 (SCALEPLUSRECT[0] + 5, SCALEPLUSRECT[1] +
                  (SCALEPLUSRECT[3] / 2)),
                 (SCALEPLUSRECT[0] + SCALEPLUSRECT[2] - 5,
                  SCALEPLUSRECT[1] + (SCALEPLUSRECT[3] / 2)),
                 1)
# draw scale negative button
scale_neg_rect = pygame.draw.rect(screen, BUTTONCOLOR, SCALENEGRECT, 2)
pygame.draw.line(screen, BUTTONCOLOR,
                 (SCALENEGRECT[0] + 5, SCALENEGRECT[1] +
                  (SCALENEGRECT[3] / 2)),
                 (SCALENEGRECT[0] + SCALENEGRECT[2] - 5,
                  SCALENEGRECT[1] + (SCALENEGRECT[3] / 2)),
                 1)
# draw scale line
pygame.draw.line(screen, GREEN,
                 (SCALELINERECT[0], SCALELINERECT[1] + (SCALELINERECT[3] / 2)),
                 (SCALELINERECT[0] + SCALELINERECT[2],
                  SCALELINERECT[1] + (SCALELINERECT[3] / 2)),
                 1)
pygame.draw.line(screen, GREEN,
...
pygame.draw.line(screen, GREEN,
                 ...
                 (SCALELINERECT[0] + SCALELINERECT[2],
                  SCALELINERECT[1] + SCALELINERECT[3]),
                 1)
scale_text = myfont.render(calc_scale_km(scale, SCALELINERECT[2]), 1, BUTTONCOLOR)
...
# startup the radar
radar = Radar(screen, RADARRECT, radar_pos=(home_x, home_y),
              scale=scale, back_col=BLACK, radar_col=GREEN)
radar.start()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if scale_plus_rect.collidepoint(mousepos):
                ...
                scale_text = myfont.render(calc_scale_km(
                    scale, SCALELINERECT[2]), 1, GREEN)
                screen.blit(scale_text, (SCALETEXTRECT[0], SCALETEXTRECT[1]))
            ...
            if dot_found != None:
                ...
                text = myfont.render(
                    radar.dots[dot_found].data.squawk, 1, GREEN)
                screen.blit(
                    text, (FLIGHTDATAPOS[0], FLIGHTDATAPOS[1] + LINESPACE))
                text = myfont.render(
                    radar.dots[dot_found].data.flight, 1, GREEN)
                screen.blit(
                    text, (FLIGHTDATAPOS[0], FLIGHTDATAPOS[1] + (LINESPACE * 2)))
                text = myfont.render(
                    "lat: " + str(radar.dots[dot_found].data.lat), 1, GREEN)
                screen.blit(
                    text, (FLIGHTDATAPOS[0], FLIGHTDATAPOS[1] + (LINESPACE * 3)))
                text = myfont.render(
                    "lon: " + str(radar.dots[dot_found].data.lon), 1, GREEN)
                screen.blit(
                    text, (FLIGHTDATAPOS[0], FLIGHTDATAPOS[1] + (LINESPACE * 4)))
                text = myfont.render(
                    "alt: " + str(radar.dots[dot_found].data.altitude), 1, GREEN)
                screen.blit(
                    text, (FLIGHTDATAPOS[0], FLIGHTDATAPOS[1] + (LINESPACE * 5)))
                text = myfont.render(
                    "spd: " + str(radar.dots[dot_found].data.speed), 1, GREEN)
                screen.blit(
                    text, (FLIGHTDATAPOS[0], FLIGHTDATAPOS[1] + (LINESPACE * 6)))
                # start the flight data timer
                if flight_data_timer != None:
                    flight_data_timer.cancel()
                flight_data_timer = Timer(
                    FLIGHTDATATIMEOUT, pygame.draw.rect, (screen, BLACK, FLIGHTDATARECT, 0))
